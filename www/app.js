...
function normalizeDb(x){
  ...
  x.classes.forEach(c=>{
    ...
    c.curriculumId=c.curriculumId||`TCCC-TIER${c.tierId||''}`;
...
function makeAttempt(c,s,attemptNo){
  ...
  return {
    ...
    trainerSign:c.leadEvaluator||'',evaluatorId:c.evaluatorId||'',studentSign:'',overallNotes:'',showNt:false,appVersion:APP_VERSION,
...
function createClass(data){
  ...
  const c={id:uuid(),name:data.name.trim(),tierId:data.tierId,roster:data.roster.trim(),scenario:data.scenario.trim(),scenarioVersion:data.scenarioVersion.trim()||'1',date:data.date,location:data.location.trim(),siteCode:String(data.siteCode||'').trim(),courseType:String(data.courseType||'initial'),scenarioDifficulty:String(data.scenarioDifficulty||'standard'),scenarioProfile:String(data.scenarioProfile||'').trim(),leadEvaluator:data.leadEvaluator.trim(),evaluatorId:String(data.evaluatorId||'').trim(),curriculumId:`TCCC-TIER${data.tierId}`,createdAt:now(),status:'draft',closedAt:null,closedBy:'',deletedAt:null,scenarioNT:[],students:[],contentVersion:`${source.source} | app ${APP_VERSION}`,tierSnapshot:deep(source)};
  db.classes.unshift(c); saveDb(); return c;
}
...
function rosterMatches(s){
  ...
  if(q&&!`${s.name||''} ${s.rank||''} ${s.trainingId||''}`.toLowerCase().includes(q))return false;
...
function renderClass(){
  ...
  $('classMeta').innerHTML=[['Roster',c.roster||'—'],['Course type',String(c.courseType||'initial').replace(/-/g,' ')],['Scenario',`${c.scenario||'—'} · v${c.scenarioVersion||'1'}`],['Scenario difficulty',String(c.scenarioDifficulty||'standard').replace(/-/g,' ')],['Scenario profile',c.scenarioProfile||'—'],['Date',c.date||'—'],['Location',c.location||'—'],['Site code',c.siteCode||'—'],['Lead Evaluator',c.leadEvaluator||'—'],['Evaluator ID',c.evaluatorId||'—'],['Curriculum ID',c.curriculumId||`TCCC-TIER${c.tierId}`],['Content',c.contentVersion||t.source],['Status',life],['Closed',c.closedAt?`${new Date(c.closedAt).toLocaleString()}${c.closedBy?` · ${c.closedBy}`:''}`:'—']].map(([a,b])=>`<div class="metaCell"><small>${esc(a)}</small><b>${esc(b)}</b></div>`).join('');
...
function newClassForm(existing=null){
  ...
  $('formModalBody').innerHTML=`<form id="classForm"><div class="formGrid"><label><span>Class name *</span><input name="name" required value="${esc(existing?.name||'')}"></label><label><span>Tier *</span><select name="tierId" ${existing?'disabled':''}>${tOpts}</select></label><label><span>Roster / course #</span><input name="roster" value="${esc(existing?.roster||'')}"></label><label><span>Course type</span><select name="courseType">${courseOpts}</select></label><label><span>Date</span><input name="date" type="date" value="${esc(existing?.date||new Date().toISOString().slice(0,10))}"></label><label><span>Installation / location</span><input name="location" value="${esc(existing?.location||'')}"></label><label><span>Site code / standardized location ID</span><input name="siteCode" value="${esc(existing?.siteCode||'')}" placeholder="e.g., HIK-01"></label><label><span>Lead evaluator</span><input name="leadEvaluator" value="${esc(existing?.leadEvaluator||'')}"></label><label><span>Evaluator ID / cadre identifier</span><input name="evaluatorId" value="${esc(existing?.evaluatorId||'')}"></label><label><span>Scenario</span><input name="scenario" value="${esc(existing?.scenario||'')}"></label><label><span>Scenario version</span><input name="scenarioVersion" value="${esc(existing?.scenarioVersion||'1')}"></label><label><span>Scenario difficulty</span><select name="scenarioDifficulty">${diffOpts}</select></label><label class="full"><span>Scenario profile / key conditions</span><textarea name="scenarioProfile" rows="2" placeholder="Optional standardized scenario descriptors">${esc(existing?.scenarioProfile||'')}</textarea></label></div><div class="formActions"><button class="action primary" type="submit">${existing?'Save':'Create Class'}</button></div></form>`;
  $('classForm').onsubmit=e=>{e.preventDefault();const f=new FormData(e.currentTarget);if(existing){['name','roster','date','scenario','scenarioVersion','location','siteCode','courseType','scenarioDifficulty','scenarioProfile','leadEvaluator','evaluatorId'].forEach(k=>existing[k]=String(f.get(k)||'').trim());saveDb();closeModal('formModal');renderClass();}else{const c=createClass(Object.fromEntries(f.entries()));closeModal('formModal');openClass(c.id);}};
  openModal('formModal');
}
function addStudentForm(){
  if(isClassClosed()){alert('Closed classes are permanently locked.');return;}
  $('formModalTitle').textContent='Add Student';
  $('formModalBody').innerHTML=`<form id="studentForm"><div class="formGrid"><label><span>Name *</span><input name="name" required></label><label><span>Rank</span><input name="rank"></label><label class="full"><span>Training ID / privacy-safe identifier</span><input name="trainingId"></label></div><div class="formActions"><button class="action primary" type="submit">Add Student</button></div></form>`;
...
function itemMarkup(item){
  const st=evalState(),r=st.ratings[item.id]||'', forced=!!st.timerForced[item.id], linked=timersForItem(item.id), hide=r==='nt'&&!st.showNt&&!item.critical,reviewMode=!st.fieldMode;
...
async function exportIndividualCsv(){
  ...
  const h=['record_type','class_id','class_name','class_status','closed_at','roster','course_type','site_code','scenario','scenario_version','scenario_difficulty','lead_evaluator','evaluator_id','student','rank','training_id','tier','curriculum_id','attempt','remediation_reason','remediation_action','result','score_percent','criterion_id','section','critical','provenance','daf_supplemental','grade','grading_method','graded_at','failure_mode','failure_mode_label','primary_contributor','primary_contributor_label','contributing_factor','failure_comment','criterion_note','source_reference','timer_id','timer_label','timer_instance','timer_standard','timer_grading_clock','timer_wall_start','timer_wall_stop','timer_duration_seconds','timer_result','tactical_pause_seconds','admin_pause_seconds','timer_voided','timer_void_reason','event_at','event_elapsed_seconds','event_label','event_detail','overall_notes','trainer_sign','evaluator_id','student_sign','app_version'];
  const records=[];const base={class_id:c.id,class_name:c.name,class_status:lifecycleStatus(c),closed_at:iso(c.closedAt),roster:c.roster,course_type:c.courseType||'',site_code:c.siteCode||'',scenario:c.scenario,scenario_version:c.scenarioVersion,scenario_difficulty:c.scenarioDifficulty||'',student:stu.name,rank:stu.rank,training_id:stu.trainingId,tier:t.id,curriculum_id:st.curriculumId||c.curriculumId||`TCCC-TIER${t.id}`,attempt:st.attemptNo,remediation_reason:st.remediation?.reason||'',remediation_action:st.remediation?.action||'',result:st.finalResult||p.result,score_percent:p.score.denom?(p.score.percent*100).toFixed(1):'',overall_notes:st.overallNotes||'',trainer_sign:st.trainerSign||'',evaluator_id:st.evaluatorId||c.evaluatorId||'',student_sign:st.studentSign||'',app_version:APP_VERSION};
  records.push({...base,record_type:'SUMMARY'});
  t.sections.forEach(sec=>sec.items.forEach(i=>records.push({...base,record_type:'CRITERION',criterion_id:i.id,section:sec.code,critical:i.critical?'YES':'NO',provenance:i.provenance||'source',daf_supplemental:i.provenance==='daf'?'YES':'NO',grade:st.ratings[i.id]||'',grading_method:st.methods[i.id]||'',graded_at:iso(st.stamps[i.id]),failure_mode:st.failureDetails?.[i.id]?.mode||'',failure_mode_label:st.failureDetails?.[i.id]?.modeLabel||'',primary_contributor:st.failureDetails?.[i.id]?.contributor||'',primary_contributor_label:st.failureDetails?.[i.id]?.contributorLabel||'',contributing_factor:st.failureDetails?.[i.id]?.contributingFactor||'',failure_comment:st.failureDetails?.[i.id]?.comment||'',criterion_note:st.notes[i.id]||'',source_reference:i.sourceReference||'',curriculum_id:st.curriculumId||c.curriculumId||`TCCC-TIER${t.id}`})));
...
function buildClassSummaryPdf(){
  ...
  opRect(ops,0,710,612,82,'#0B2A48');opText(ops,30,758,'TCCC CLASS PERFORMANCE SUMMARY',18,true,'#FFFFFF');opText(ops,30,738,`${c.name} | Tier ${t.id} ${t.shortName} | ${lifecycleStatus(c)}`,11,true,'#CFFAFE');opText(ops,30,721,`Roster ${c.roster||'-'} | ${c.scenario||'Scenario'} v${c.scenarioVersion||'1'} | ${c.date||''} | ${c.location||''}`,8,false,'#E2E8F0');
...
async function exportClassSummaryCsv(){const c=cls(),t=tier();const rows=[['class_id','class_name','class_status','closed_at','roster','course_type','site_code','scenario','scenario_version','scenario_difficulty','student','rank','training_id','tier','curriculum_id','attempt','remediation_reason','remediation_action','finalized','result','pass_count','fail_count','nt_count','no_count','score_percent','started_at','finalized_at']];c.students.forEach(s=>Object.values(s.attempts||{}).filter(Boolean).sort((a,b)=>a.attemptNo-b.attemptNo).forEach(a=>{const ss=scoreStats(t,a);rows.push([c.id,c.name,lifecycleStatus(c),iso(c.closedAt),c.roster,c.courseType||'',c.siteCode||'',c.scenario,c.scenarioVersion,c.scenarioDifficulty||'',c.leadEvaluator||'',a.evaluatorId||c.evaluatorId||'',s.name,s.rank,s.trainingId,t.id,a.curriculumId||c.curriculumId||`TCCC-TIER${t.id}`,a.attemptNo,a.remediation?.reason||'',a.remediation?.action||'',a.finalizedAt?'YES':'NO',a.finalResult||proficiencyFor(t,a).result,ss.pass,ss.fail,ss.nt,ss.no,ss.denom?(ss.percent*100).toFixed(1):'',iso(a.startedAt),iso(a.finalizedAt)]);}));await saveFile(`TCCC_${safe(c.name)}_Summary.csv`,bytes(csv(rows)),'text/csv');}
...
function proficiencyFor(t,st){const saveC=currentClassId,saveS=currentStudentId,saveA=currentAttemptNo; // static scoring without timer aggregate, used only export summary fallback
  const items=t.sections.flatMap(s=>s.items),ss=scoreStats(t,st),cf=items.filter(i=>i.critical&&st.ratings[i.id]==='fail').length,un=items.filter(i=>!st.ratings[i.id]||st.ratings[i.id]==='no'||(i.critical&&st.ratings[i.id]==='nt')||criticalFailureCauseMissing(i,st)).length;return {result:un?'INCOMPLETE':cf||ss.percent<.75?'FAIL':'PASS'};
}
async function exportClassCriteriaCsv(){const c=cls(),t=tier();const rows=[['class_id','class_name','class_status','closed_at','roster','course_type','site_code','scenario','scenario_version','scenario_difficulty','lead_evaluator','evaluator_id','student','rank','training_id','tier','attempt','criterion_id','section','critical','provenance','daf_supplemental','grade','grading_method','graded_at','failure_mode','failure_mode_label','primary_contributor','primary_contributor_label','contributing_factor','failure_comment','note','source_reference','curriculum_id','app_version']];c.students.forEach(s=>Object.values(s.attempts||{}).filter(Boolean).forEach(a=>t.sections.forEach(sec=>sec.items.forEach(i=>rows.push([c.id,c.name,lifecycleStatus(c),iso(c.closedAt),c.roster,c.courseType||'',c.siteCode||'',c.scenario,c.scenarioVersion,c.scenarioDifficulty||'',c.leadEvaluator||'',a.evaluatorId||c.evaluatorId||'',s.name,s.rank,s.trainingId,t.id,a.attemptNo,i.id,sec.code,i.critical?'YES':'NO',i.provenance||'source',i.provenance==='daf'?'YES':'NO',a.ratings[i.id]||'',a.methods[i.id]||'',iso(a.stamps[i.id]),a.failureDetails?.[i.id]?.mode||'',a.failureDetails?.[i.id]?.modeLabel||'',a.failureDetails?.[i.id]?.contributor||'',a.failureDetails?.[i.id]?.contributorLabel||'',a.failureDetails?.[i.id]?.contributingFactor||'',a.failureDetails?.[i.id]?.comment||'',a.notes[i.id]||'',i.sourceReference||'',a.curriculumId||c.curriculumId||`TCCC-TIER${t.id}`,a.appVersion||APP_VERSION])))));await saveFile(`TCCC_${safe(c.name)}_Criteria.csv`,bytes(csv(rows)),'text/csv');}
async function exportClassTimersCsv(){const c=cls(),t=tier();const rows=[['class_id','class_name','class_status','closed_at','course_type','site_code','scenario','scenario_version','scenario_difficulty','evaluator_id','student','training_id','tier','attempt','timer_id','timer_label','linked_criterion','instance','standard','grading_clock','wall_start','wall_stop','duration_seconds','result','tactical_pause_seconds','admin_pause_seconds','voided','void_reason']];c.students.forEach(s=>Object.values(s.attempts||{}).filter(Boolean).forEach(a=>(t.timers||[]).forEach(d=>{const store=a.timers?.[d.id]||{instances:[]};store.instances.filter(x=>x.wallStart).forEach(x=>{const dur=durationStatic(d,x);rows.push([c.id,c.name,lifecycleStatus(c),iso(c.closedAt),c.courseType||'',c.siteCode||'',c.scenario,c.scenarioVersion,c.scenarioDifficulty||'',a.evaluatorId||c.evaluatorId||'',s.name,s.trainingId,t.id,a.attemptNo,d.id,d.label,d.linkedItemId||'',x.index,d.standard,d.gradingClock,iso(x.wallStart),iso(x.wallStop),(dur/1000).toFixed(1),x.voided?'VOID':resultStatic(d,x),((x.tacticalPauseMs||0)/1000).toFixed(1),((x.adminPauseMs||0)/1000).toFixed(1),x.voided?'YES':'NO',x.voidReason||'']);});})));await saveFile(`TCCC_${safe(c.name)}_Timers.csv`,bytes(csv(rows)),'text/csv');}
async function exportClassEventsCsv(){const c=cls(),t=tier();const rows=[['class_id','class_name','class_status','closed_at','course_type','site_code','scenario','scenario_version','scenario_difficulty','evaluator_id','student','rank','training_id','tier','attempt','event_at','event_elapsed_seconds','event_label','event_detail','app_version']];c.students.forEach(stu=>Object.values(stu.attempts||{}).filter(Boolean).forEach(a=>(a.events||[]).forEach(e=>rows.push([c.id,c.name,lifecycleStatus(c),iso(c.closedAt),c.courseType||'',c.siteCode||'',c.scenario,c.scenarioVersion,c.scenarioDifficulty||'',a.evaluatorId||c.evaluatorId||'',stu.name,stu.rank,stu.trainingId,t.id,a.attemptNo,iso(e.at),((e.elapsed||0)/1000).toFixed(1),e.label||'',e.detail||'',a.appVersion||APP_VERSION]))));await saveFile(`TCCC_${safe(c.name)}_Events.csv`,bytes(csv(rows)),'text/csv');}
...
$('classPdfBtn').onclick=exportClassPdf;$('classSummaryCsvBtn').onclick=exportClassSummaryCsv;$('classCriteriaCsvBtn').onclick=exportClassCriteriaCsv;$('classTimersCsvBtn').onclick=exportClassTimersCsv;$('classEventsCsvBtn').onclick=exportClassEventsCsv;$('classAnalyticsCsvBtn').onclick=exportClassAnalyticsCsv;$('backupClassBtn').onclick=backupClass;$('closeClassBtn').onclick=closeClass;$('deleteClassBtn').onclick=deleteClass;
